        ...
        <table id="datatable-facility-overview"
            ...
            @php($totalMonthlyPerGrained = (($lease -> capital_rent_per_month_base_currency + $lease ->
                ...
                <tr>
                    ...
                    <td>{{ mYFormat(rnd($lease -> total_lease_base_currency)) }}</td>
                    <td>{{ mYFormat(rnd($lease -> capital_rent_per_month_base_currency)) }}</td>
                    <td>{{ mYFormat(rnd($lease -> maintenance_rent_per_month_base_currency)) }}</td>
                    <td>{{ mYFormat(rnd($totalMonthlyPerGrained)) }}</td>
                    <td>{{ mYFormat(rnd($lease -> square_metres)) }}</td>
                    <td>{{ mYFormat(rnd($lease -> grained_surface_area)) }}</td>
                    <td>{{ mYFormat(rnd($lease -> number_of_employees,0),0) }}</td>
                    <td>{{ $lease -> employees_last_updated }}</td>
                    <td>{{ mYFormat(rnd($lease -> number_of_workstations,0),0) }}</td>
                    <td>{{ $lease -> workstations_last_updated }}</td>
                    <td>{{ floatval($lease -> square_metres) ? mYFormat(rnd($totalMonthlyPerGrained/$lease -> square_metres)):0 }}</td>
                    <td>{{ $lease -> number_of_employees ? mYFormat(rnd($totalMonthlyPerGrained/$lease -> number_of_employees)):0 }}</td>
                    <td>{{ $lease -> number_of_workstations ? mYFormat(rnd($totalMonthlyPerGrained/$lease -> number_of_workstations)):0 }}</td>
                    <td>{{ $lease -> number_of_employees ? mYFormat(rnd($lease -> square_metres / $lease -> number_of_employees)) : 0}}</td>
                    <td>{{ mYFormat(rnd($lease -> parking_cost_per_month_base_currency)) }}</td>
                    <td>{{ mYFormat(rnd($lease -> number_of_parking_spaces,0),0) }}</td>
                    <td>{{ $lease -> number_of_parking_spaces ? mYFormat(rnd($lease -> parking_cost_per_month_base_currency / $lease -> number_of_parking_spaces)) : 0}}</td>
                    <td>{{ $lease -> counterparty -> short_name }}</td>
                    <td>{{ mYFormat(rnd($lease -> other_cost_per_month_base_currency )) }}</td>
                    <td>{{ $lease -> renovation_and_rent_free_periods }}</td>
                    <td>{{ mYFormat(rnd($lease -> rent_security_deposit_base_currency )) }}</td>
